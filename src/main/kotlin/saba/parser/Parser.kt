...
class Parser(val lexer: Lexer) {
    ...
    private fun parseFunctionLiteral(): Expression? {
        val currentToken = checkedCurrentToken()
        if (!expectPeek(TokenType.LPAREN)) return null
    ...
    private fun parseFunctionParameters(): List<Identifier> {
        ...
        nextToken()
        identifiers.add(Identifier(checkedCurrentToken(), checkedCurrentToken().literal, ""))
        while (peekTokenIs(TokenType.COMMA)) {
            nextToken()
            nextToken()
            identifiers.add(Identifier(checkedCurrentToken(), checkedCurrentToken().literal, ""))
        }
        if (!expectPeek(TokenType.RPAREN)) { // 参考書ではnullを返してるだけだけどidentifiers返してみるか…
